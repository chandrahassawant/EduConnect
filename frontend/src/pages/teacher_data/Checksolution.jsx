...
export default function Checksolution() {
    ...
    useEffect(() => {
        const fetchAssignments = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:3100/api/teacher/check-assignments', {
                ...
                if (Array.isArray(response.data)) {
                    // Set specific elements or handle array as needed
                    setAssignmentId(response.data[1]?.assignmentId || ''); // Adjust based on actual data structure
                    setFilePath(response.data[7]?.filePath || ''); // Adjust based on actual data structure
    
                    // Log state variables to ensure they are set correctly
                    console.log(filePath);
                    console.log(assignmentId);
                    setFilePath(assignments.data[7].filepaths);
    
                    // Update state with the assignments and filepaths
                    setState(prevState => ({
                        ...prevState,
    ...
    }, [state.subjectId, state.userId]); // Ensure this runs when `subjectId` or `userId` changes
    ...
    // Handle change in comments or marks
        ...
        const pendingCheckFetch = async () => {
          try {
            const response = await axios.get(
              ...
              { userId }  // Assuming userId is a primitive value
            );
    ...
    return (
        <Card sx={{ minWidth: 275 }}>
            ...
            >
                ...
                <Button style={{ marginLeft: "70%" }} variant="contained">Checked</Button>
            </Stack> */}

            {/* Assignment Table */}
            <TableContainer component={Paper}>
                <Table sx={{ minWidth: 650, textAlign: "center", marginTop: "3%" }} size="medium" aria-label="a dense table">
                    <TableHead>
                        <TableRow>
                                ...
                                <TableRow key={row.assignment_id}>
                                    ...
                                    <TableCell align="center">
                                        <TextField
                                            ...
                                            onChange={(e) => handleInputChange(e, row.assignment_id, 'assignment_comments')}
                                            variant="outlined"
                                            size="small"
                                        />
                                    </TableCell>
                                    <TableCell align="center">
                                        <TextField
                                            ...
                                            onChange={(e) => handleInputChange(e, row.assignment_id, 'assignment_marks')}
                                            variant="outlined"
                                            size="small"
                                            type="number"
                                        />
